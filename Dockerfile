...
RUN apt-get update && (apt-get install -y --no-install-recommends zip unzip curl docker-compose-plugin socat docker.io docker-compose python3-pip nodejs python3-venv jq bash git 2>&1|grep -v "Get:") && (apt-get clean all ||true) && which jq && which git && which curl  
